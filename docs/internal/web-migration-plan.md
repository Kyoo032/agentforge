...
| Locale is one value for the whole install | `packages/host/src/settings-store.ts:121` (`locale?: AppLocale`), exported from `packages/core/src/index.ts:520-521` | **Copy and catalogues unchanged.** Only the storage of the chosen locale moves to per-user | 4 |
...
**Files.** `packages/core/src/tenancy/types.ts:13-20` gains `tenantId`; `packages/host/src/tenant.ts:26`
...
`packages/host/src/handlers/media.ts:39`); a path-traversal filename cannot escape the tenant prefix;
