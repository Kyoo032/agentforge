...
| The one branch point | `packages/host/src/http-adapter.ts:347-358` inside `mutatingRejection` | Done. Server mode → origin+host+CSRF; else the loopback rule. |
...
| Gate fails closed on the server | `packages/host/src/gateway-gate.ts:276` (`serverMode`), `:278-280` (stub runtime no longer opens it), `:288-290` (unverified key = `error`, not `ok`) | Done. |
