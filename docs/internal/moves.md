...
- **Kept:** the Electron desktop app, frozen at 0.14.27 and maintenance-only — the pack and release routes, the `Kyoo032/DPSBuddy` releases repo and the `0.14.2x` version scheme all continue for desktop maintenance cuts. Gateway-first is unchanged, and so are the webdev `:3000` rules.
