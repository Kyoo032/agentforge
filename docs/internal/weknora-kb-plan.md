...
**Packaging (how it reaches the user).** Nothing is installed separately and nothing is downloaded at install or runtime. One new component ships inside the existing installer: the WeKnora-lite static Go binary + `migrations/sqlite/` + `LICENSE` / `THIRD_PARTY_NOTICES.md` / `licenses/`, staged into `apps/desktop/resources/weknora/` (gitignored except README, like `resources/ffmpeg/`) and copied by electron-builder `extraResources` to `process.resourcesPath/weknora/`. Not shipped and not needed: Docker, Postgres, Redis, Neo4j, Python docreader, a Go runtime. NSIS copies plain files (no custom step, no admin); `deleteAppDataOnUninstall` already removes `%APPDATA%\DPSBuddy\weknora`; auto-update carries the sidecar with the full installer. Build lane: GitHub Actions matrix at the pinned WeKnora commit (`windows-latest` + MinGW → x64 exe; `macos-latest` → arm64 + x64), artifacts published as a release we own, pulled by `apps/desktop/scripts/stage-weknora.mjs` before packaging so no dev box needs Go/MinGW/Rust. The mac Docker lane on the Windows box cannot produce a CGO mac binary; a `zig cc` cross-compile is the fallback spike. Unsigned helpers launch today like ffmpeg; once the app is signed the Go binary is signed + notarized on the same path. Expected cost: ~+100 MB installer, RAM/cold-start per Spike 2; if Windows CGO is the hard lane, ship builtin-only on Windows and WeKnora on mac behind the same UI.
...
### 2026-09-11 — Phase 3 code landed against a fake sidecar; binary build blocked on Actions

**State:** host backend (`packages/host/src/knowledge/backends/weknora/`), registry with degraded fallback, outbox, backfill, routes, migration 0013, doctor fields, web backend card and `features/knowledge-backend.md` all in and green against a fake WeKnora HTTP server plus a fake Node "binary". Contract suite runs against both backends. Integration suite exists and is skipped until `AGENTFORGE_WEKNORA_BIN` points at a real build. Verified on :3100 without a binary: `GET /knowledge` reports the sidecar unavailable, `PUT {id:"weknora"}` → 409 `backend_unavailable (not_staged)`, builtin retrieval unchanged (`· hybrid`), no process spawned.

**Build lane:** `.github/workflows/weknora-lite.yml` + `scripts/stage-weknora.mjs` + `apps/desktop/weknora.lock.json` are committed. Every workflow in this private repo fails at startup (a one-line canary too) while the account's public repo runs fine → private-repo Actions is locked at the account level (billing / spending limit). Decision pending with the owner: a separate public build repo (free, incl. macOS minutes) vs unblocking private-repo billing. The public DPSBuddy releases repo is ruled out as the home: it is the auto-update feed.

**Corrections to the plan learned from the WeKnora source:** the port is `SERVER_PORT` (viper AutomaticEnv over `server.port`), the AES key is `SYSTEM_AES_KEY` (exactly 32 bytes; `TENANT_AES_KEY` is dead), `ENABLE_GRAPH_RAG` / `WEKNORA_SANDBOX_MODE` are read nowhere (`NEO4J_ENABLE` is the gate), health is `GET /health` outside `/api/v1`, manual ingest has no metadata field so it is `POST …/manual` then `PUT /knowledge/:id {custom_metadata}`, and metadata comes back on hits as rendered `key: value` lines. `LOCAL_STORAGE_BASE_DIR` defaults to an absolute `/data/files` and must be set.

**Found while building / in review:** (a) `replaceSourceRows` and `deleteSource` dropped the row before the remote delete → orphaned docs on every re-index; `external_id` now travels with the row. (b) The supervisor reported `running` before readiness, so a second caller during the 20 s boot got a dead URL. (c) No total deadline on the retrieval path — a cold start could block Chat for a minute; now a 5 s race, builtin answers. (d) Builtin vectors were skipped while WeKnora was healthy, making the fallback FTS-only; builtin now always indexes first. (e) Port-race: between picking a free port and the child binding it, any local process could impersonate the sidecar and receive the gateway key; the listening socket's owner PID is now checked against the child before the first credential-bearing call. (f) Response bodies were buffered before the size cap. (g) Migration 0013 carried a bare `ALTER TABLE` that the ensure-schema mirror already owns; removed. (h) The gateway key lives AES-encrypted in the sidecar's own SQLite under `%APPDATA%`; it is now revoked on switch-off and on key change.
