...
Renderer only, both products. The host still decides the gate; the desk only displays `allowed`, and treats host `status: "stub"` as no live model so a cold stub walk does not fire a send that cannot succeed. Nothing packed.

- **Send and Generate stay quiet without a live model.** Chat Send, Documents Generate, and Images, Videos, and Music Generate do not start a request when the host reports `stub` or `allowed: false`. Settings is the only primary action. The key sentence sits on the control. Sign out and Delete everything stay in Settings, behind a disclosure (`settings-reset-danger`), for a desk that already has a key as well as one that does not. A no-key Settings intro is the short paste-the-key line; the spend cap stays hidden until a key is saved.
