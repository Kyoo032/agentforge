...
- **2026-09-26** — Finance is a guided path in the renderer both products share, on the cold-desk header (mode icon, one outcome line, live cards). `/finance` with no task query asks “What do you want to do?” and offers five plain cards. A task then asks for a file or typed numbers, with one primary under the inputs: Read my numbers, then Write it up (an empty instruction uses the task’s default). A result’s primary is Download. The optional note, the model, and How this works sit in More options. The studio uses the stage lane (`--content-stage`). The rail group stays Make, and the Finance chevron still reads “tasks” and starts closed. Figures are still computed in core; import limits, PII masking, and the export routes are unchanged. An unknown `?task=` still opens the brief. Driven on stub webdev. Not packed.
- **2026-09-26** — Each Finance task's catalog sample (and a plain sentence in that shape) now reads into figures. Cash flow ("Opening cash… Jan in…, out…"), budget ("Budget: … Actual: …") and ratios ("Current assets …, debt …, EBITDA …") were rejected by the sheet readers; core rewrites only that shape into the ledger those readers already accept, and `cellValue` still reads every amount. The brief and appraisal samples take the same path and no longer need a model to name the rows. The budget box hint is the sample sentence. Not packed.
- **2026-09-26** — Chat is more alive for someone who is not watching a log. A placeholder mascot (`PlaceholderMascot`, `data-placeholder="nultron-mascot"`) sits on the empty desk and beside a live turn: idle, thinking, answering, error. The SVG is stand-in art in the desk accent — the owner is drawing the Nultron character and swaps that file. A paragraph that is only a link, file, image, or job URL becomes a card (`message-embed`), text-file attachments inlined as `--- name ---` do too, and a search hit or titled tool payload shows as a result card under the answer. Message appear, the empty desk, those cards, and the send button use the existing motion scale and stop under `prefers-reduced-motion`. The composer keeps `shrink-0` so it stays put while the thread scrolls. Copy is in both chat catalogs. On the redesigned desk the idle mascot sits in the hero orb and the live one uses the desk gradient and lip; cards use `card-live`. Creating a thread no longer looks like switching sessions, so the live reply actually draws and Send comes back. A refused send shows its sentence on `composer-error` only; `chat-error` stays for a pane that could not open, and the live mascot goes to `error`. Not packed. Map: [`maps/chat-send.md`](maps/chat-send.md).
- **2026-09-26** — Onboarding is three steps on the current desk: a short gradient hello in the in-app name (DPSBuddy), the Toko Token key with plain success and failure copy, then four example cards (chat, a document, finance, images) that open Chat. The gateway address is not on that screen. The component installer stays on the key step and does not block it. The screen scrolls itself (`h-full overflow-y-auto`), the same full-page rule as sign-in, because the document no longer scrolls. The host still decides `allowed`; there is no password and no sign-in form. Copy is in `en` and `id`. Not packed. Maps: [`settings-and-gateway-gate.md`](maps/settings-and-gateway-gate.md), [`component-installer.md`](maps/component-installer.md).
- **2026-09-26** — Research no longer asks for a Tavily or Brave key. A saved key is still used when one is present; otherwise search is Wikipedia (the desk locale), OpenAlex, then arXiv and Crossref when that pair is thin or the query looks scholarly. Origins are pinned, redirects stay on that origin, queries are PII-masked, and results are cached in the host process for 15 minutes. DuckDuckGo HTML, Mojeek HTML, public SearXNG, the Marginalia `public` key, and Jina Reader are not used. Both products: the tool lives in core and the renderer is shared. Unit tests mock HTTP. One live `photosynthesis` query from this environment returned Wikipedia "Photosynthesis" and an OpenAlex DOI. Nothing packed, nothing deployed. Map: [`maps/research-dossier.md`](maps/research-dossier.md).
...
- **Both — the XLSX Calc sheet disagreed with the report.** Cash flow: the Inputs sheet wrote the raw
  ...
  (`blankUnless`, `packages/core/src/finance/ratios/formulas.ts:237`).
