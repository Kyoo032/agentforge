...
(`packages/host/src/router.ts:219`) hands the table back and `isByIdRoute` (`:210`) filters it, and
...
(`packages/host/src/router.ts:493`) with the **verified session's** tenant id — never the
