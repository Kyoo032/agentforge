...
> `apps/web/locales/{en,id}/{settings,onboarding}.json` are modified and uncommitted. Every other cite
> on this page is committed code.
...
The literal `"__default__"` appears exactly once in the repo, at `:24`.
...
**`settingsPayload()`** (`packages/host/src/handlers/settings.ts:83-120`) is what the renderer sees. Keys are
...
- Onboarding renders one muted line, `onboarding-gateway-host` → `onboarding.gatewayHost` = `"Gateway: {host}"`
  (`apps/web/components/onboarding-screen.tsx:119-121`), so the screen now has exactly one input — the key.
...
**What a save does besides saving.** `handlePostSettings` (`packages/host/src/handlers/settings.ts:162-176`)
...
| Call | Why |
|---|---|
| `clearThisKeyCache()` (`:164`) | the per-key usage strip is keyed on the old credential |
| `resetEmbedCircuit()` (`:167`) | the embeddings breaker holds a workspace down for five minutes |
| `resetJobModelCircuit()` (`:168`) | **added 2026-09-17** — the job-model fallback breaker skips a model for the same five minutes (`JOB_MODEL_DOWN_MS`, `packages/host/src/job-model-fallback.ts:25`), so a save that fixes the key must clear it too or the desk keeps routing around a model that now works |
| `revokeKnowledgeGatewayModel(...)` (`:172-176`) | the retrieval sidecar holds a *copy* of the key inside the model row it embeds with; a key changed here but left in that row has not been rotated |
...
| no verdict, or verdict for a different fingerprint | `{status:"ok", allowed:true, grace:true, message:"Not checked yet."}` — **opened on trust** | `:273-275` |
| verdict `ok`, fresh (< `GATEWAY_OK_TTL_MS`) | `allowed:true, grace:false` | `:277-282` |
| verdict `ok`, stale | `allowed = withinGrace(lastOkAt, …)`, `grace:true` | `:283-284` |
| verdict `invalid_key` | `allowed:false, grace:false` — **no grace, ever** | `:287-289` |
| verdict `unreachable` / `error` | `allowed = withinGrace(…)`, and `grace` mirrors `allowed` | `:290-293` |
| persisted `stub` / `needs_key` verdict | treated as never-checked | `:295` |

Constants (`packages/host/src/gateway-gate.ts`): `GATEWAY_GRACE_MS = 7 * 86_400_000` (`:33`),
`GATEWAY_CHECK_TIMEOUT_MS = 3_000` (`:36`), `GATEWAY_OK_TTL_MS = 86_400_000` (`:42`),
`GATEWAY_REFRESH_THROTTLE_MS = 600_000` (`:45`), `GATEWAY_UNCHECKED_MESSAGE = "Not checked yet."` (`:48`),
`GATEWAY_VERDICT_UNWRITABLE_MESSAGE` (`:55`). The grace boundary is inclusive — exactly 7 days still counts
(`withinGrace`, `:236-241`, pinned at `packages/host/src/gateway-gate.test.ts:146-155`); the TTL check is
`withinOkTtl` (`:242-248`).

**The live check** is `checkGatewayLive` (`packages/host/src/gateway-gate.ts:311-347`): `GET {baseUrl}/models`
...
other non-OK → `error`; thrown/timeout → `unreachable`. `runGatewayCheck` (`:424-469`) wraps it: it
short-circuits on stub or no key (`:431-433`), maps a throw to `unreachable` (`:445-449`), carries `lastOkAt`
forward only for the same fingerprint (`:451`), persists, and — when the verdict could **not** be written —
answers `status: "error"` with `allowed` untouched (`:464-468`).

It runs on a key save (`refreshGatewayGateAfterSave` → `runGatewayCheck`,
`packages/host/src/handlers/settings.ts:237-250`, with `gateVerdictFor` at `:226-235` deciding which verdict the
save response carries) and on `POST /api/v1/settings/gateway/check` (`handleGatewayCheck`, `:254-262`, route at
`packages/host/src/router.ts:177`). Separately, `maybeRefreshGateway` (`packages/host/src/gateway-gate.ts:503-533`)
...
**Enforcement.** `requireGatewayAllowed(settings)` (`packages/host/src/gateway-gate.ts:411-417`) throws
...
`handlers/knowledge.ts:149, 166, 182, 231, 266, 288, 348, 381`; `handlers/finance.ts:16, 28, 42, 53`;
...
**Renderer.** `parseGatewayGate` (`apps/web/lib/gateway-gate.ts:50-74`) requires a known `status` and a boolean
`allowed` or returns `null`; `MISSING_GATEWAY_GATE` fails closed. `resolveGate(payload, isElectron)` (`:83-89`)
obeys a reported gate as-is, and on a missing or malformed one falls closed **only under Electron** — webdev and
Playwright keep working before the host answers. `App.tsx` holds `gate` / `gateway` / `localeEpoch` state
(`apps/web/src/App.tsx:75-77`), listens for `GATE_EVENT = "agentforge-gate"`
(`apps/web/lib/gateway-gate.ts:97`, listener at `App.tsx:86-95`), re-fetches settings and re-resolves on every
locale epoch (`:97-121`), and renders `OnboardingScreen` when `gate === "onboarding"` (`:138-139`). Reason copy
maps through `REASON_KEYS` (`apps/web/lib/gateway-gate.ts:112-116`) to
`onboarding.gate.invalidKey / unreachable / error`; the Settings status word maps through `gatewayStatusKey`
(`:124-126`) to `settings.gateway.status.<status>`.
...
(`packages/host/src/handlers/settings.ts:301-302`, 400 otherwise). Then
`requestDataReset(localDataDir(), [...HOST_RESET_ENTRIES])` (`:305`) writes the marker — nothing is deleted yet,
because "the database is open and ffmpeg may still be writing" (`:304`) — followed by `killTrackedChildren()`,
...
**The wipe list is named entries, never the directory** (`packages/host/src/handlers/settings.ts:270-283`),
...
`packages/host/src/handlers/settings.ts:267-268`), and, when `DATABASE_URL` points out of tree, that trio by
absolute path (`packages/db/src/reset.ts:188-242`). Pinned exactly by
`packages/host/src/handlers/settings.test.ts:345-358`, which also asserts `host-status.json` and anything
containing "storage" never appear (`:360-361`). Preserved: `host-status.json`, `Local Storage/`, every other
...
`apps/web/server-env.ts:14` (webdev) and `apps/desktop/main.cjs:620` inside `bootstrapPackaged()`, before
...
| Case | Behaviour |
|---|---|
| `gateway-gate.json` missing, unreadable, or not JSON | `loadGateState` returns `null` (`packages/host/src/gateway-gate.ts:126-140`) → opens on trust |
| Verdict cannot be written | `saveGateState` warns and returns `{persisted:false}` (`:161-185`, warn at `:172-184`); `runGatewayCheck` then answers `status:"error"` with `allowed` as derived (`:464-468`) — "an unwritable data dir must not close a desk" |
| Live check throws / times out | mapped to `unreachable` (`:445-449`), never thrown |
| Background re-check throws | swallowed (`:525-531`); the un-awaited promise also carries its own `.catch` (`:528`) |
| Save with an empty key string | `clearGateState()` and no check at all (`packages/host/src/handlers/settings.ts:244-247`) |
...
- **AGENTS.md's `resolveProviderKeys` precedence is Phase 1 design, not shipped code.** `AGENTS.md:72`
...
- **`getTenant` reads and compares the settings file on every single request** because `adoptLegacySettings` is
  unconditional (`packages/host/src/tenant.ts:33`). It is a no-op once the legacy slice is gone, but it is not
  free.
...
`onboarding-setup-check` `:109`, `onboarding-form` `:118`, `onboarding-gateway-host` `:119`,
`onboarding-gate-reason` `:123`, `onboarding-key` `:136`, `onboarding-continue` `:145`, `onboarding-recheck`
`:155`. **`settings-endpoint`, `settings-endpoint-reset`, `openai-base-url` and `onboarding-endpoint` must all
...
**Why the gate is advisory and every rule fails open.** `[Direct]` `AGENTS.md:67`, written in the same commit
...
(`packages/host/src/gateway-gate.ts:126-140`), an unwritable data dir costs a cached decision and nothing else
(`:161-185`, `:464-468`), the background re-check swallows its own failures (`:525-531`), and the stub runtime is
always open (`:262-264`). **Confidence: high.** The design record is prose in `AGENTS.md`, not a commit body —
...
`packages/host/src/handlers/settings.ts:264-269` — "Deliberately a named list, never the directory: in the packaged
app this same folder is Electron's userData / Chromium profile, so `host-status.json`, `Local Storage/`, caches
and cookies are not ours to delete"; and `:304` — "the database is open and ffmpeg may still be writing, so the
wipe is queued for the next boot". `[Supported]` `packages/host/src/handlers/settings.test.ts:345-361` pins the
