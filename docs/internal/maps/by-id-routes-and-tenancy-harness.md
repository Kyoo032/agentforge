...
Last verified: 2026-09-20 at d14cd8f + branch feat/web-phase3-lane-e-j6rp29 (the commit that adds this page)
...
prefixes and per-tenant settings are lane D and are not done.

## How it works

### The surface, counted

`packages/host/src/router.ts:191` is one array of `compile(method, path, handler)` rows. At
`d14cd8f` it holds **132 registrations, 52 of them by-id** — a route is by-id when `compile`
(`router.ts:157`) extracted at least one `:param` name into `keys`.

The Phase 3 spec counted "129 route registrations, 55 of them by-id"
([`../web-phase3-tenancy-spec.md`](../web-phase3-tenancy-spec.md) §5) against the Phase 2 tree.
Both halves moved: three routes were added (the Music mode, PR #65) and the by-id count is lower
because the spec's figure counted a route once per `:param` on a handful of two-param paths. 52 is
the count of by-id *registrations*, which is the unit the harness iterates.
...
| The handler lists what the tenant owns and looks in that list | `handleSelectWorkspace` (`packages/host/src/handlers/workspaces.ts:97`) | 404 |
...
The fourth shape does not 404, and that is correct rather than a gap: an answer that is the same
whether or not the row exists tells a prober less than a 404 does. What it needs instead of a 404 is
proof the row was not touched, which the harness gives (below).
...
`getTenant(request)` — **104 sites across 20 handler files**. The change is mechanical and changes
...
every kind the `:params` name (`seedTenantA`, `:368`), each carrying `LEAK_MARKER` (`:53`) in its
text fields. Tenant B then calls all 52 by-id routes with A's ids. Per route:
...
2. **No existence oracle.** The same route is called again with ids nothing has ever had (`fillPath`
   in `"ghost"` mode, `:540`) and the two answers must reduce to the same string (`shape`, `:597`,
...
3. **404**, on every route except the five that carry `refusal: "indistinguishable"` and a written
   reason (the `Refusal` type, `:83`).
...
`routeRegistrations()` (`router.ts:182`) hands back the route table with no handlers attached;
`isByIdRoute` (`:187`) filters it. The harness re-derives the by-id list from that and fails if the
router has a by-id route the table does not (`:687`). A by-id route added without a tenancy test
...
`dispatch` opens `withLogContext({ tenantId, route })` around the handler (`router.ts:424`) with the
...
the store, so the desktop's and webdev's lines are unchanged. This is security spec row L1's tenant
half; the request id is PR #71's A09-1 and composes with it.
...
| `packages/host/src/handlers/*.ts` | The 104 swept call sites |
| `packages/db/src/ensure-local-owner.ts` | `createLocalWorkspace`, and its `ownerUserId` argument |

## Gotchas

- **A 404 is not the only correct refusal.** Five routes answer something else on purpose and each
...
- **`GET /api/v1/videos/examples/:name/file` is not tenant data.** `listVideoExamples`
  ...
  binary. It is in the table, called and asserted, because leaving a by-id route out of the table is
  exactly what the completeness assertion exists to prevent — but there is no owner for it to be
  foreign to.
...
- **`createLocalWorkspace`'s desk-member default was a hosted 500.** It wrote the membership row for
  ...
  `handlePostWorkspaces` passes `tenant.userId` (`packages/host/src/handlers/workspaces.ts:77`). The
