...
The rail tab is `mode-presentations` (`mode-${href.slice(1)}`), and `/presentations` is one of the work-mode routes that renders **`null`** in the router (`apps/web/src/App.tsx:163`). The real component is mounted by `WorkModeKeepAlive`, which maps `/presentations` → `PresentationsStudio` (`apps/web/components/work-mode-keep-alive.tsx:29`) and keeps every visited mode mounted, hidden when inactive (`:48-54`). Consequence, verified on the desk: switch to Chat and back and the loaded deck is still there; reload and it is gone.
...
`onGenerate` (`apps/web/components/presentations-studio.tsx:51-76`) POSTs `{ prompt, model?, sourceText? }` through `apiFetch`, so webdev and the packaged app share one call. Route table: `packages/host/src/router.ts:284`.
...
Submit uploads any held image through `POST /api/v1/media`, inlines any held `.txt` into the instruction (`:96-107`), then `onRegenerate` POSTs the **whole outline** plus `slideIndex` to `/api/v1/presentations/regenerate` (`apps/web/components/presentations-studio.tsx:86-98`; route `packages/host/src/router.ts:285`).
...
| `packages/host/src/router.ts:284-286` | The three routes |
