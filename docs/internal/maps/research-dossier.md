...
All three model calls go through `collectJobAssistantText` (`packages/host/src/job-regen.ts:66-144`) with `jobMode: "research"` and `withOutputLanguage(system, "research", localeForRun())`, and all three are parsed with `extractJsonObject` + `JSON.parse`; a non-JSON answer is `invalid_research` / 502 (`research-dossier.ts:77-83`).
...
| Model call itself failed | `collectJobAssistantText`, `packages/host/src/job-regen.ts:140-142` | `job.error {code:"generation_failed", status:502}` |
