...
**3. Translation is a second pass over the finished JSON, not a second reading of the transcript.** `translateMinutes` (`packages/host/src/meeting/run.ts:274`) hands the model the minutes object and asks for the same shape in the other language, so the two languages cannot disagree about what was decided. This is on top of the repo's usual `withOutputLanguage` surface rule, which only decides what language a single call writes in.

## How it works

### 1. Reaching the surface

`mode-meeting` on the left rail is built from the product mode's href (`packages/core/src/agents/product-modes.ts:11`, `href: "/meeting"`), so there is no literal `data-testid="mode-meeting"` to grep — the rail composes it (`apps/web/components/app-rail.tsx:316`). `/meeting` itself is `<Route path="/meeting" element={null} />` (`apps/web/src/App.tsx:163`); the studio mounts through `WorkModeKeepAlive` (`apps/web/components/work-mode-keep-alive.tsx:28`), not through the route element.
...
**Why 25 MB and not more:** the HTTP adapter refuses any request body over `MAX_BODY_BYTES` = 26 MB (`packages/host/src/http-adapter.ts:29-30`). A larger cap in the store would only turn a clear 413 into a confusing transport failure. Roughly an hour of speech-grade mono audio fits; a longer meeting goes in as a pasted transcript.
...
`transcribeMeeting` (`packages/host/src/meeting/run.ts:138`):
...
3. `transcribeChunks` (`packages/host/src/meeting/transcribe.ts:191`) posts each chunk in order, emitting a `job.step` per chunk.
...
Then the guard. `guardMinutesNames` (`packages/core/src/meeting/guard.ts:54`) checks every name the minutes assert against the transcript:
