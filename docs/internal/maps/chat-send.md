...
`readRunStream` (`apps/web/components/chat-composer.tsx:113-155`) reads the body, feeds `consumeSse` (`apps/web/lib/sse-client.ts:14-33`), and dispatches: `run.started` / `run.probing` → `onStarted`; `assistant.delta` → accumulate `streaming`; `assistant.thinking` → accumulate `thinking`; `tool.started` / `tool.completed` → the tool list; `run.failed` → `onFailed` + `composer-error`. A refused HTTP response takes the same path through `fail` (`:273-277`). `ChatSession`'s `onFailed` (`chat-session.tsx:481-484`) clears `running` and sets `sendFailed` so the live mascot goes to `error`. It does not copy the sentence into `chat-error`. That banner is for a pane that could not open.

While anything is live, or a send just failed, `ChatTurn` renders inside a `data-testid="assistant-live"` wrapper (`apps/web/components/chat-session.tsx:418`): `PlaceholderMascot` sits beside the turn (`apps/web/components/chat-turn.tsx:89`) in `thinking` until answer text arrives, then `answering`, or `error` when `sendFailed` is set. The thinking `<details>` shows `thinking-placeholder` until the first thinking token (`apps/web/components/chat-turn.tsx:108`), tools render as `message-tools` (`apps/web/components/chat-turn.tsx:117`), and the answer renders through `FormattedText` as `message-output` (`apps/web/components/chat-turn.tsx:135`). A paragraph that is only a link, a file URL, or a host image becomes a card in `presentStandaloneLink` (`apps/web/components/formatted-text.tsx:63`). A markdown image whose source must not auto-load (`![alt](https://…)`) is an image card with no `<img>` (`formatted-text.tsx:54`), marked `fromImage` on the link (`apps/web/lib/parse-markdown.ts`). Search hits and a titled tool payload become job cards under the answer (`apps/web/components/chat-turn.tsx:145`). On `onComplete` the live stream state is cleared and `refreshMessages` does a `GET /api/v1/threads/:id` (`apps/web/components/chat-session.tsx:501`), after which the same text re-renders from the persisted message — through a different component, with the same `message-output` testid (`apps/web/components/chat-turn.tsx:219`, `:254`). A failed send keeps the error mascot up (`sendFailed`) until the next send or a thread switch. The empty desk shows the same mascot at `idle` inside the hero orb (`apps/web/components/chat-launcher.tsx:100`).
...
| `apps/web/components/chat-turn.tsx` | One turn: thinking `<details>`, tools, `message-output`, live mascot, job cards |
| `apps/web/components/placeholder-mascot.tsx` | Placeholder Nultron mascot SVG. Swap the art; states live in `lib/mascot-states.ts` |
| `apps/web/components/message-embed.tsx` | Link, file, image, video, and job cards in a turn |
| `apps/web/lib/message-embeds.ts` | Which standalone links, file blocks, and tool payloads become cards |
...
DOM testids that prove it: `composer` / `composer-text` / `composer-send` / `composer-error` (`apps/web/components/chat-composer.tsx:460`, `:506`, `:636`, `:550`), `message-list`, `assistant-live` (`apps/web/components/chat-session.tsx:418`), `message-thinking` / `thinking-placeholder` / `message-tools` / `message-output` (`apps/web/components/chat-turn.tsx:94`, `:110`, `:117`, `:135`), `chat-mascot` (`apps/web/components/placeholder-mascot.tsx:26`), `message-embed` (`apps/web/components/message-embed.tsx:69`), `model-picker` / `model-picker-panel` (`apps/web/components/model-picker.tsx:441`, `:382`). Packaged proof needs `doctor.mjs --desktop` (`transport: "ipc"`), not `:3000`. A refused send shows `composer-error` and not `chat-error`.
