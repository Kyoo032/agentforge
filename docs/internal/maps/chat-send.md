...
6. The generator yields whatever `send()` pushed (`packages/host/src/runs.ts:439-445`). Each frame is `encodeSse(event)` — `event: <type>\ndata: <json>\n\n` (`packages/core/src/sse.ts:6`).
