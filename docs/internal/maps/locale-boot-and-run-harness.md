...
(`packages/host/src/settings-store.ts:402`), stored as the top-level `locale` field of the settings envelope
(`packages/host/src/settings-store.ts:119`). There is deliberately **no env override**: `AGENTFORGE_LOCALE` is
...
overwrites and "Settings Restart is the only product caller" (`apps/web/lib/i18n.ts:121`).
...
`Record<AppLocale, Record<Namespace, MessageTree>>` (`apps/web/lib/i18n.ts:65-108`). 19 namespaces (`:41-61`):
...
`t("namespace.path.to.key", vars?)` (`apps/web/lib/i18n.ts:179-186`) resolves through `lookup` (`:149-166`) and
...
| `packages/host/src/settings-store.ts:402-411` | `loadOwnerLocale` / `saveOwnerLocale` |
