...
2. `handlePostSettings` (`packages/host/src/handlers/settings.ts:203-207`) validates and calls
...
`BLOCKED_MESSAGES` (`packages/host/src/gateway-gate.ts:421-430`) — "English, redacted, and specific enough that a
...
| Case | Behaviour |
|---|---|
| Unknown locale value posted | 400 `invalid_request` from `isAppLocale` (`packages/host/src/handlers/settings.ts:212-213`) |
...
| `packages/host/src/settings-store.ts:461-465` | `loadOwnerLocale` / `saveOwnerLocale` |
