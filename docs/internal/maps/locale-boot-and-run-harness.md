...
| videos | `packages/host/src/studio-generate.ts:225` |
...
image* (`packages/host/src/studio-generate.ts:167`); **Presentation** → `presentationLanguageRule` spliced into
...
(`packages/host/src/job-regen.ts:132`) sets `locale: options.locale ?? localeForRun()` on the `runtime.execute`
...
| `packages/host/src/job-regen.ts:132` | Where `locale` joins `runtime.execute` for every job |
