...
- **The coming-soon panel is dead code today.** Every task is `available: true`, so `finance-task-coming-soon` never renders and `finance_task_unavailable` is never thrown. Do not write a recipe around either without first re-reading `tasks.ts`.
