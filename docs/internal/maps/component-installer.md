...
- **Start over removes it.** `components` and `logs` are in `HOST_RESET_ENTRIES` (`packages/host/src/handlers/settings.ts:354`, the two entries at `:290-292`); the wipe runs on the next boot before the module is loaded, so Windows never holds the `.node` open.
