...
Host side, `router.ts:221` → `handlePostDocuments` (`packages/host/src/handlers/jobs.ts:110-119`): `getTenant`, then `requireGatewayAllowed(loadSettings(...))` **before** anything else, then `generateDocumentDraft`.
...
4. `resolveDocumentModel` (`:125-133`) — body `model`, else `settings.documentGenModel`, else the mode default, resolved against the live catalog by `resolveChatModel`. The Documents bucket **is** the chat bucket (`packages/host/src/selectable-models.ts:145`) and the default ladder is `["hy3", "hy-3", "hunyuan-3", "hunyuan3", "deepseek-v4-flash"]` (`packages/core/src/models/mode-defaults.ts:47`) — Finance's is the same list minus one alias (`:57`), which is why both land on `deepseek-v4-flash` on a gateway without Hunyuan.
...
9. If that saved, `upsertWorkSource(artifactWorkCard({ type: "Documents", … }))` files a knowledge work card (`:165-170`, `packages/host/src/work-cards.ts:129`, `:168`).
...
|---|---|---|
| Gate closed | `requireGatewayAllowed`, `packages/host/src/handlers/jobs.ts:114`, `:125` | HTTP 403 `gateway_blocked` on generate and regenerate. **`/documents/docx` has no gate and still returns bytes** |
...
| `packages/host/src/router.ts:221-223` | The three route registrations |
...
| testid | Source |
|---|---|
| `mode-documents` | `apps/web/components/app-rail.tsx:316` (built as `mode-${href.slice(1)}`) |
