...
Host side, `router.ts:221` → `handlePostDocuments` (`packages/host/src/handlers/jobs.ts:173-182`): `getTenant`, then `requireGatewayAllowed(loadSettings(...))` **before** anything else, then `generateDocumentDraft`.
