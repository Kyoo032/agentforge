...
- **Settings are per workspace.** `settings.enc` is `{ workspaces: { <id>: StoredSecrets } }` (`packages/host/src/settings-store.ts:119-123`). A key saved on one desk is not a key on another, and `clearGatewayKeyEverywhere` (`:375-390`) exists precisely because the per-workspace shape makes "remove my key" a multi-row operation.
