...
Plaintext on purpose: `threads.title` (`packages/db/src/schema.ts:372`, written unsealed at `packages/host/src/threads.ts:43` and `:252-255`), `agent_versions.model` (`packages/db/src/schema.ts:318`), `runs.usage` / `runs.error` (`packages/db/src/schema.ts:410-411`, written unsealed at `packages/host/src/threads.ts:322-325`), and the key fingerprints themselves.
...
- **`piiWarning()` is dead code.** It exists (`packages/core/src/security/pii.ts:299-314`), is exported (`packages/core/src/index.ts:148`) and is tested (`packages/core/src/security/pii.test.ts:197-211`), but no product code calls it. It is the leftover of the removed banner; `pii-warning` and `pii-send-anyway` are 0 everywhere in `apps/` and `packages/`.
