...
Plaintext on purpose: `threads.title` (`packages/db/src/schema.ts:226`, written unsealed at `packages/host/src/threads.ts:41` and `:244-247`), `agent_versions.model` (`packages/db/src/schema.ts:172`), `runs.usage` / `runs.error` (`packages/db/src/schema.ts:264-265`, written unsealed at `packages/host/src/threads.ts:313-316`), and the key fingerprints themselves.
...
- **No host job calls `maskPii`, and every host job is still masked.** Grep `packages/host` for `maskPii` and you find studio prompts (`packages/host/src/studio-generate.ts:167`, `:224`), edit retries (`packages/host/src/edit/generate.ts:38`), search queries (`packages/host/src/research-generate.ts:146`, `packages/host/src/market/tools.ts:84`) and knowledge cards (`packages/host/src/knowledge-ingest.ts:32`, `:44`) — and none of Chat, Finance, Data, Documents, Presentations, Market or Legal. That absence used to read as "host jobs are not masked". It is the opposite: **every** job goes through `collectJobAssistantText` (`packages/host/src/job-regen.ts:87-88`), which builds the runtime with `createRuntime(settings)` and therefore lands on the same wrapper Chat does (`packages/core/src/runtime/create-runtime.ts:26`). Finance reaches it from `finance-generate.ts` and `finance-tasks/runner.ts`. The mask is a property of the runtime, not of any one call site — which is why nobody has to remember to call it.
...
- **`piiWarning()` is dead code.** It exists (`packages/core/src/security/pii.ts:196-211`), is exported (`packages/core/src/index.ts:110`) and is tested (`packages/core/src/security/pii.test.ts:88-101`), but no product code calls it. It is the leftover of the removed banner; `pii-warning` and `pii-send-anyway` are 0 everywhere in `apps/` and `packages/`.
