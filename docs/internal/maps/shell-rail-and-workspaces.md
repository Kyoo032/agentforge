...
- **Creating a desk selects it.** `handlePostWorkspaces` calls `writeSelectedWorkspaceId` before returning 201 (`packages/host/src/handlers/workspaces.ts:75`), which is why the page can go straight to `/chat`. There is no "create without switching".
