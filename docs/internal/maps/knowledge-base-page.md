...
- **Empty Soul fields are silently replaced.** `putSoul` substitutes `DEFAULT_SOUL.name` / `.role` / `.voice` for anything that trims to empty (`packages/host/src/knowledge.ts:155-159`), so "clear the voice and save" is not a thing a user can do from this page.
