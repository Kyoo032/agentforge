...
All three are gated (`requireGatewayAllowed`, `packages/host/src/handlers/knowledge.ts:378`, `:409`) because all three may need an embedding call. All three end in `await reload()`, which is why a new row and the loop numbers land together.
