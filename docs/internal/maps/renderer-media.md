...
`GET /api/v1/media/:mediaId/file` → `handleGetMediaFile` (`packages/host/src/handlers/media.ts:26-50`), routed at `packages/host/src/router.ts:214`, with HTTP `Range` support via `packages/host/src/byte-range.ts`. It is **ungated** — media is on the list of routes that stay open so a closed gate is always recoverable. Its only scoping is `getTenant(request.workspaceId)` plus an `organizationId` match on the row.
