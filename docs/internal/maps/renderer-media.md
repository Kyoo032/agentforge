...
In a Chat run (`packages/host/src/runs.ts:343-351`), every `tool.completed` output goes through the host collector, then each part through `mirrorToolMediaPart` (`:450-465`):
...
- **A part that cannot be mirrored is dropped, not persisted** (`packages/host/src/runs.ts:461-464`): "the turn keeps its text and drops the picture rather than carrying a remote URL."
...
The Images and Videos studios do the same thing directly (`packages/host/src/studio-generate.ts:178-179`, `:239-240`).
...
| `packages/host/src/runs.ts:450-465` | `mirrorToolMediaPart` for Chat runs |
| `packages/host/src/studio-generate.ts:178-179, 239-240` | The same mirroring for Images / Videos |
