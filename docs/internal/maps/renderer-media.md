...
In a Chat run (`packages/host/src/runs.ts:348-356`), every `tool.completed` output goes through the host collector, then each part through `mirrorToolMediaPart` (`:453-468`):

- `image_url` → `saveGeneratedImage`, `video_url` → `saveGeneratedVideo` (`packages/host/src/media.ts:132-183`). An already-local `/api/v1/media/…` URL is returned as-is; a `data:` URL is decoded directly (`decodeMediaDataUrl`, `:125-131`); anything else goes to `downloadGeneratedMedia`.
...
- The bytes go to `saveMedia` (`packages/host/src/media.ts:67-108`), which re-validates mime and size, stores the object through the per-tenant store (`putTenantObject`, `packages/host/src/tenant-storage.ts:447-464` — the media root on a desk, a COS bucket on a server that sets `AGENTFORGE_STORAGE=cos`, and the tenant's quota checked before the write), inserts a `media` row scoped to `tenant.organizationId`, and returns `/api/v1/media/${id}/file`. An upload over the ceiling is refused with `storage_quota_exceeded` ([`tenant-object-storage.md`](tenant-object-storage.md)).
- **A part that cannot be mirrored is dropped, not persisted** (`packages/host/src/runs.ts:475-478`): "the turn keeps its text and drops the picture rather than carrying a remote URL."
...
`GET /api/v1/media/:mediaId/file` → `handleGetMediaFile` (`packages/host/src/handlers/media.ts:25-51`), routed at `packages/host/src/router.ts:307`, with HTTP `Range` support via `packages/host/src/byte-range.ts`. It is **ungated** — media is on the list of routes that stay open so a closed gate is always recoverable. Scoping is `getTenant(request)` plus an `organizationId` match on the row, and then the key itself: `readTenantObjectRange(tenant.tenantId, item.storagePath, …)` (`packages/host/src/tenant-storage.ts:594-600`) refuses a `storage_path` that is not the caller's with the same 404 — a pure string check before any IO, plus symlink resolution on the file backend — so a row whose `storage_path` was tampered with cannot serve another tenant's bytes ([`tenant-object-storage.md`](tenant-object-storage.md), [`tenant-storage.md`](tenant-storage.md)).
...
| `IMAGE_MAX` / `VIDEO_MAX` (on save) | 10 MB / 50 MB | `packages/host/src/media.ts:10-16` |
| Allowed image mimes | `image/png`, `image/jpeg`, `image/webp`, `image/gif` | `packages/host/src/media.ts:10-16` |
| Allowed video mimes | `video/mp4`, `video/webm`, `video/quicktime` | `packages/host/src/media.ts:10-16` |
...
| `packages/host/src/runs.ts:464-479` | `mirrorToolMediaPart` for Chat runs |
