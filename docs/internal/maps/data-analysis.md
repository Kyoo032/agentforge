...
`createDatasetStore` (`packages/host/src/datasets.ts:195-311`) is a repository over the kernel `datasets` table (`packages/db/src/schema.ts:805-820`, mirrored for older DBs at `packages/db/src/ensure-schema.ts:374-391`). `create` (`:208-253`):

1. `parseOrThrow` (`:135-152`) — 25 MB cap → 413, unparseable → 413, no header/data row → 400, more than `DATASET_MAX_ROWS = 200_000` rows → 413.
2. Writes the raw bytes to `<datasetRoot()>/<workspaceId>/<uuid><ext>` (`:207-225`); `datasetRoot()` is `localDataDir()/datasets` (`:315-317`).
...
| `packages/host/src/router.ts:294-299` | The six data routes |
...
| `packages/db/src/schema.ts:805-820`, `packages/db/src/ensure-schema.ts:374-391` | The `datasets` table and its back-fill |
