...
| H3 | `/data` mounted `nodev`, and `noexec` once the component installer's native modules load from the image path instead of `/data/components` (today the image bundles `anydoc` and reports it `ready`, so the first-boot download path is unused; keep it that way) | Phase 7 | `mount` |
...
|---|---|---|---|
| A1 | Mutating `/api` accepts only a configured trusted-origin allowlist; a **missing Origin is rejected** on the web adapter (today it is allowed, `packages/host/src/local-request.ts:79-82`). The second check on the same path, `isLoopbackHostHeader` at `http-adapter.ts:187-188`, must accept the configured public host too; until then the proxy rewrites `Host` to `127.0.0.1` (`webapp-deploy/Caddyfile`) | **before traffic** (Phase 1) | `local-request.ts`, `http-adapter.ts:185-198` |
| A2 | CSRF: session cookie `SameSite=Lax`, `Secure`, `HttpOnly`, plus a double-submit token on every non-GET | Phase 1 | `http-adapter.ts` |
...
| A7 | Uploads: MIME is sniffed from magic bytes, not taken from the client's `file.type` (`packages/host/src/media.ts:29-42`); `/api/v1/media/:id/file` sends `Content-Type` from the stored sniffed type, `X-Content-Type-Options: nosniff`, and `Content-Disposition: attachment` for anything that is not an image or video | Phase 6 | `media.ts`, media handler |
...
| T2 | `getTenant()` derives the tenant from the session, never from a client-supplied id; every by-id handler filters by tenant (102 call sites, `packages/host/src/tenant.ts:26`) | Phase 3 | `tenant.ts` and all handlers |
...
| T4 | The gateway gate **fails closed** on the hosted build: a missing or malformed gate means blocked (today it fails open off Electron, `apps/web/lib/gateway-gate.ts:83-89`) and the host stops taking an unverified key on trust (today `packages/host/src/gateway-gate.ts:273-275` returns `allowed: true` when there is no state or the fingerprint changed) | **before traffic** | both files |
| T5 | Plan check runs inside `requireGatewayAllowed` (`packages/host/src/gateway-gate.ts:411`, 34 call sites, the one choke point): Personal `active` with allowance left, or Enterprise `active` with a seat for this user; otherwise 403 with the reason code from the portal list | Phase 5 | `gateway-gate.ts` |
...
| T8 | "Start over" is scoped to the caller's tenant, or disabled on the web build; today it wipes the whole data dir (`packages/host/src/handlers/settings.ts:273-321`) | **before traffic** | `settings.ts` |
| T9 | Per-tenant caps on concurrent ffmpeg children, SQL workers and runs. Today there is **no concurrency cap at all**: `packages/host/src/child-processes.ts` only tracks children to kill them on quit, and `sql-runner.ts:8-11` caps rows, time and cell size per query, not how many run at once | Phase 5 (a global cap **before traffic**) | those files |

## 6. Secrets and encryption

| # | Requirement | When | Where |
|---|---|---|---|
| S1 | `AGENTFORGE_SECRETS_KEY` is mandatory on the server; the `.master-key` file fallback throws when `NODE_ENV=production` (`packages/db/src/vault-key.ts:37-51`) | **before traffic** (Phase 1) | `vault-key.ts` |
