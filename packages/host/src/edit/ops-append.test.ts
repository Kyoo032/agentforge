...
describe("ops append (G-01)", () => {
  it("rejects agent ops without cardId", async () => {
    ...
    await addReadyClip(project.id, project.workspaceId, "clip-g01");
    await expect(
      appendOps(project.id, [{ type: "set_volume", payload: { clipId: "clip-g01", volume: 0.5 } }], { actor: "agent:run-1", workspaceId: project.workspaceId }),
    ).rejects.toMatchObject({ code: "card_required", status: 400 });
  });

  it("assigns monotonic seq", async () => {
    const { project } = await seedEditProject("seq");
    await addReadyClip(project.id, project.workspaceId, "clip-seq");
  ...
  it("snapshots every 200 ops", async () => {
    ...
    await addReadyClip(project.id, project.workspaceId, "clip-snap");
    const start = (await foldProject(project.id, project.workspaceId)).seq;
    ...
    const result = await appendOps(project.id, batch, { actor: "owner", workspaceId: project.workspaceId });
  ...
  it("accepts agent ops when cardId is present", async () => {
    ...
    await addReadyClip(project.id, project.workspaceId, "clip-agent");
    const result = await appendOps(
      project.id,
      [{ type: "set_volume", payload: { clipId: "clip-agent", volume: 1.2 }, cardId: "card-1" }],
      { actor: "agent:run-ok", cardId: "card-1", workspaceId: project.workspaceId },
    );
...
describe("ops append errors", () => {
  it("surfaces invalid payloads as ApiError", async () => {
    ...
    await expect(
      appendOps(project.id, [{ type: "set_volume", payload: { clipId: "missing", volume: 9 } }], { actor: "owner", workspaceId: project.workspaceId }),
