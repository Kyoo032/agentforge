/**
 * Independent checker for `appraisal-solar-en`. Written against the FILE: it re-opens the generated
 ...
 * method, not by the bisection build.mjs used, NPV at the IRR recorded in case.json is checked to be ~0,
 * and NPV(r) is scanned to confirm the three sign changes in the flows still leave exactly one IRR.
 * It never imports build.mjs and never imports the finance engine.
 *
 * Run from `packages/host`:  node eval/finance/cases/appraisal-solar-en/verify.mjs
 */
...
const HEADER_LABEL = "Line item";
const SUBTOTAL_LABEL = "Net cash flow";
...
/** First non-negative crossing only — Year 5 pushes the running total back down before it gets there. */
...
const flowSignChanges = signs.filter((sign, i) => i > 0 && sign !== signs[i - 1]).length;
if (flowSignChanges < 2) {
  record(`the mid-life negative year is gone: only ${flowSignChanges} sign change(s) in the flows`);
}
if (FLOWS.slice(1).every((flow) => flow >= 0)) {
  record("no negative year among Years 1..N; the payback trap is gone");
}
...
expectFigure(figures, "flowSignChanges", flowSignChanges);
expectFigure(figures, "npvZeroCrossings", crossings);
...
process.stdout.write(
  [
    ...
    `cumulative ${cumFlows.map((total) => total.toFixed(0)).join(" | ")}`,
    `NPV @ ${caseJson.params.discountRate}% ${baseNpv.toFixed(2)}, IRR (secant) ${(baseIrr * 100).toFixed(6)}%`,
    `flow sign changes ${flowSignChanges}, NPV(r) zero crossings ${crossings} -> the IRR is unique`,
    `NPV at the recorded IRR ${npvAtRecordedIrr.toExponential(3)} (band +/-${zeroBand.toExponential(3)})`,
...
if (failures.length > 0) {
  process.stderr.write(`FAIL appraisal-solar-en\n${failures.map((line) => `  - ${line}`).join("\n")}\n`);
  process.exit(1);
}
process.stdout.write("OK appraisal-solar-en\n");
