...
- **Maps:** [`docs/internal/maps/`](docs/internal/maps/README.md) — 22 pages, one per subsystem, each with a `Last verified: <date> at <sha>` line and `file:line` citations. Read the page before working in a subsystem; if your change makes it false, refresh it in the same PR (rewrite in place, bump the verified line) and refresh the matching `features/<feature>.md`. Product bugs a mapping pass finds go to `unreleased.md` as findings, not into the map PR as fixes.
...
- `@firecrawl/anydoc` is a native module loaded through `createRequire`; esbuild leaves it as a runtime require, so it must exist as a real `node_modules` entry outside the asar in the packaged app. A green webdev says nothing about the installer here.
