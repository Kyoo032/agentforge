...
- **Maps:** [`docs/internal/maps/`](docs/internal/maps/README.md) — 27 pages, one per subsystem, each with a `Last verified: <date> at <sha>` line and `file:line` citations. Read the page before working in a subsystem; if your change makes it false, refresh it in the same PR (rewrite in place, bump the verified line) and refresh the matching `features/<feature>.md`. Product bugs a mapping pass finds go to `unreleased.md` as findings, not into the map PR as fixes.
