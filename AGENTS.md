...
- Artifacts: `apps/desktop/dist/DPSBuddy Setup 0.14.26.exe` + mac arm64/x64 dmg/zip (gitignored). Anything older in `dist/` (`published-*`, `superseded-*`) is history, not current.
...
- `docs/internal/unreleased.md` is titled “after public v0.14.25” although 0.14.26 is published; its artifact table and log are current through 2026-09-17 and everything logged after the 0.14.26 cut is 0.14.27 material. Start the file over when `docs/internal/0.14.27-changelog.md` is created rather than appending under the stale header.
