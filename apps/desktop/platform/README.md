...
| Window close | Closing the window exits the whole process tree (`taskkill /T`) so `ffmpeg` and helpers die | Close hides the window and the app stays in the Dock; `activate` recreates the window; Cmd+Q quits via the `quit` role. Nothing kills child processes yet | `main.cjs` `shouldQuitOnLastWindow`, `exitApp` |
...
| Installer | NSIS x64, `build/installer.nsh`; uninstall kills the exe and deletes userData | dmg + zip, x64 and arm64, `identity: null` (unsigned, not notarized) | `package.json` `build` |
| Updater | electron-updater against `latest.yml` on DPS Agent Platform; public Agentforge flavor only | electron-updater needs a **signed** app on macOS; unsigned builds must not offer install | `auto-update.cjs` |
...
- Platform branching lives in `main.cjs` and the modules it requires. The renderer (`apps/web`) never reads `process.platform` or `navigator.platform` to change behavior; it only reads `window.agentforge` through `apps/web/lib/desktop-bridge.ts`.
