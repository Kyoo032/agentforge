...
| `make-dmg: ERROR: /DPSBuddy.app: … ['Contents']` then Frameworks helpers on 0.14.25 | `names()` required 8 `ls` columns (`Jan 01 1980`). This image prints `8/12/2026 12:17` (7 columns), and helper apps have spaces so the last token was `.app` | `parse_hfs_ls_name`: slash dates take the remainder after time; ctime dates keep the 8th column; keep a double-and-retry only for a true catalog-full |
