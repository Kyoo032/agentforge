...
- **No LAN bridge** to the desktop app. Packaged Electron has no HTTP server by design (`docs/mobile.md`). Do not add one on either side. **Premise re-scoped (2026-09-23):** the desktop is the **Personal** product, current cut `0.15.0` and under active development again, while **Enterprise** is the hosted multi-user web app with a real HTTP server — so a future phone client would target the hosted app, not a desktop on the LAN. Nothing here is being rewritten yet: the mobile plan stays **paused** (Kyo, Sept 2026) until the web app ships. See [`web-pivot-2026-09-18.md`](../../docs/internal/web-pivot-2026-09-18.md).
