...
export function PresentationPreview({
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="presentations-preview">
      <article
        className="relative aspect-video w-full overflow-hidden rounded-[16px] bg-[var(--text)] text-white"
      ...
      >
        ...
        <div className="relative flex h-full flex-col justify-center px-8 py-10 sm:px-12">
          ...
          <h2 className="mt-3 max-w-3xl text-2xl font-medium leading-tight sm:text-4xl">{outline.title}</h2>
      ...
      {outline.slides.map((slide, index) => (
        <div key={`${slide.heading}-${index}`}>
          ...
          >
            <div className="absolute inset-y-0 left-0 w-1.5 bg-[var(--text)]" />
            <div className="flex h-full flex-col px-8 py-7 sm:px-12 sm:py-10">
              ...
              <h3 className="mt-2 text-xl font-medium text-ink sm:text-3xl">{slide.heading}</h3>
