...
export function ChatContextChip({ usedTokens, contextLength, parts }: Props) {
  ...
  const rows: ContextPart[] = parts && parts.length > 0 ? parts : [{ label: "Conversation", detail: "messages in this thread", tokens: used }];
    ...
    <div ref={rootRef} className="relative ml-auto flex-none">
      ...
      >
        <svg width="20" height="20" viewBox="0 0 20 20" className="flex-none -rotate-90" aria-hidden="true">
          <circle cx="10" cy="10" r={RING_RADIUS} fill="none" stroke="color-mix(in srgb, var(--color-text) 14%, transparent)" strokeWidth="6" />
          <circle
            cx="10"
            cy="10"
            r={RING_RADIUS}
