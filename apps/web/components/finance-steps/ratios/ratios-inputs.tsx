...
function assistNotice(parsed: RatiosParsed | null): string | null {
  ...
  return error
    ? t("finance.ratios.modelAssistFailed", { asked })
    : t("finance.ratios.modelAssist", { asked, placed });
}
    ...
    >
      <div>
        ...
        <button
          ...
          className="btn mt-2"
          onClick={() => void onRead()}
          disabled={busy || !ratios.figures.trim()}
          data-testid="finance-ratios-parse"
        >
          {reading ? t("finance.ratios.parsing") : t("finance.ratios.parse")}
        </button>
      ...
      {parsed ? (
        <p className="text-xs text-[var(--text-2)]" data-testid="finance-ratios-read">
          ...
          {parsed.subtotalsIgnored > 0 ? ` ${t("finance.ratios.subtotalsIgnored", { count: parsed.subtotalsIgnored })}` : ""}
        </p>
      ) : null}
      {assistNotice(parsed) ? (
