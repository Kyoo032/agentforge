            ...
            {findings.map((finding) => (
              <tr key={finding.id} data-testid="legal-finding-row">
                <td className={TD}>
                  {finding.clause}
                  <br />
                  <span className={`text-[12px] ${DIM}`}>{finding.title}</span>
                </td>
                ...
                <td className={`${TD} font-mono text-[12px]`}>{basisLabel(finding.basis) || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <tbody>
            {findings.map((finding, index) => (
              <tr key={`${finding.clause}-${index}`} data-testid="legal-finding-row">
                ...
                <td className={`${TD} font-mono text-[12px]`}>{finding.basis || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
