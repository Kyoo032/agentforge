          ...
          <ul className="mt-1 space-y-1 text-[12px]">
            {failures.map((failure, index) => (
              <li key={`${failure.code}-${failure.target}-${index}`}>
                ...
                <span className="font-mono text-[12px]">{failure.target}</span>: {failure.detail}
          ...
          <ul className="mt-1 space-y-1 text-[12px]">
            {failedChecklist.map((item) => (
              <li key={`${item.itemId}-${item.deliverable}`}>
                <span className="font-mono text-[12px]">{item.itemId}</span> · {item.deliverable}: {item.reason}
              </li>
            ))}
          </ul>
        </Section>
        ...
        <Section title="Documents skipped">
          <ul className="mt-1 space-y-1 text-[12px]">
            {verify.documentsSkipped.map((item) => (
              <li key={item.doc}>
                <span className="font-mono text-[12px]">{item.doc}</span>: {item.reason}
              </li>
            ))}
          </ul>
        </Section>
