...
function load(locale: "en" | "id"): unknown {
  return JSON.parse(readFileSync(resolve(localesDir, locale, "videos.json"), "utf8"));
}
...
describe("videos locale JSON", () => {
  ...
  it("keeps brand names untranslated in id", () => {
    const raw = readFileSync(resolve(localesDir, "id", "videos.json"), "utf8");
