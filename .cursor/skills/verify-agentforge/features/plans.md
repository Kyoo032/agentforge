...
- **plans-page** — `/pricing` renders `pricing-page`, `pricing-placeholder` (the "Indicative pricing — not final." note, `role="note"`), and one card per tier: `pricing-tier-personal` / `pricing-tier-enterprise`, each with `pricing-price-<id>`, `pricing-features-<id>` and `pricing-cta-<id>`. `pricing-seats-<id>` is present only when that tier has a seat cap, so Personal has none.
- **plans-cta** — Personal and Enterprise do not check out from `/pricing`. Personal's control names the Mac and Windows app and does not start a seat subscription. Enterprise's control is contact DPS, a button, and a configured `AGENTFORGE_BILLING_TOPUP_URL` must not turn `pricing-cta-enterprise` into a checkout href. The current tier is a plain statement. `pricing-contact-help` stays visible and says Enterprise has no checkout.
...
**Automated proof already in the repo.** `packages/core/src/plans/catalog.test.ts` (24 — the shape, the frozen tables, `matchTier`'s refusal to guess, price formatting, and the two offer key lists), `apps/web/lib/pricing-page-render.test.tsx` (20), `plans-api.test.ts` (24), `plan-block.test.ts` (10), `plan-blocked-render.test.tsx` (8), `account-plan-render.test.tsx` (10), `plans-locale.test.ts` (4), `packages/host/src/handlers/billing.test.ts` (41). The offer split was green on 2026-09-22 for the catalog, pricing render, locale, and blocked-screen files.

## Gotchas

- **Personal has no web seat count.** `seatCap: null` still means "no cap configured", which admits everybody. The page no longer claims "One seat", so do not sign a second user into a Personal tenant and expect a refusal, and do not add a cap to match retired copy. [SR-22](../../../../docs/internal/security-register.md#sr-22) was that claim.
