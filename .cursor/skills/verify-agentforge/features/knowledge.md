...
- `knowledge-loop` (Sources tab, above Add a source) draws six stages, Work → Saved → Indexed → Graph → Retrieved → Verified → Work (`knowledge-loop-stage-<Stage>`, each with `data-value` and `data-state`), and one count bar per type (`knowledge-loop-count-<Type>`). Source counts come from the same Sources list; Graph / Retrieved / Verified come from `GET /api/v1/knowledge` (`graph`, `retrievals`, `verified`). With no sources it shows `knowledge-loop-empty`.
- `knowledge-loop-verify` ("Run self-check") POSTs `/api/v1/knowledge/verify` — a planted-fact check on the host — then reloads the page data, so the Verified stage flips to `pass` / `fail` with the time and shows the reason on hover (`knowledge-loop-verified`, `data-state`). A failure shows `knowledge-loop-verify-error`.
- `knowledge-graph-panel` (under the loop chart) is collapsed; the header carries `knowledge-graph-counts` (`data-nodes` / `data-edges`). `knowledge-graph-toggle` opens it and fetches `/api/v1/knowledge/graph?limit=200`: `knowledge-graph-loading`, then `knowledge-graph-svg` with `knowledge-graph-node-<id>` (topics left, sources middle, threads right), `knowledge-graph-legend`, and `knowledge-graph-shown` ("Showing N of M nodes"). No graph yet → `knowledge-graph-empty`; a broken fetch → `knowledge-graph-error` + `knowledge-graph-retry`. Clicking a node filters to its ego neighbourhood (`knowledge-graph-clear-focus` restores).
...
- **Loop chart and Graph.** Do this after the planted-fact recipe, so the counters have something to show.
  1. On the Sources tab `knowledge-loop-cycle` names all six stages in order: `Work`, `Saved`, `Indexed`, `Graph`, `Retrieved`, `Verified` (`knowledge-loop-stage-Graph` and `knowledge-loop-stage-Verified` exist). `knowledge-loop-stage-Retrieved` `data-value` matches the `retrievals` total from step 5 above.
  2. Click `knowledge-loop-verify` ("Run self-check"). While it runs the button reads `Checking…`. When it lands, `knowledge-loop-stage-Verified` reads `pass`, carries `data-state="ok"`, and is drawn in the pass colour with a relative time such as `just now`; hovering the stage shows the host's detail line. A `fail` is a real result, not a harness error — read the detail before retrying.
  3. Click `knowledge-tab-map` → `knowledge-map-run` ("Map knowledge"), then go back to `knowledge-tab-sources`. `knowledge-loop-stage-Graph` `data-value` is now > 0 and its sub-label counts edges. A stub map counts too.
  4. Click `knowledge-graph-toggle` ("Show graph"). `knowledge-graph-svg` draws topics on the left, sources in the middle, threads on the right, with `knowledge-graph-legend` naming the node and edge kinds. `knowledge-graph-shown` reads `Showing N of M nodes` (N ≤ 200).
  5. Click any `knowledge-graph-node-*`. The drawing filters to that node and everything one hop away (`knowledge-graph-shown` gains `· around <label>`, N drops), and `knowledge-graph-clear-focus` ("Show all") appears. Click it — or the same node again — to restore.
...
- Graph and Verified are new in this build. On an older host `GET /api/v1/knowledge` has no `graph` / `verified` fields, so those stages read `0` / `never` and `knowledge-graph-panel` shows `knowledge-graph-error` when opened (no `/api/v1/knowledge/graph` route) and `knowledge-loop-verify` fails the same way. That is an old host, not a broken page — the other four stages still count.
