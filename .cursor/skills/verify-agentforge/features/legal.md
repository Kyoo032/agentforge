...
- v1 accepts .docx only. A .txt or .pdf is refused with the exact sentence above — a refusal, not a crash, and nothing joins the matter. The studio's pre-check shows the localized `legal.errors.docxOnly` (`apps/web/components/legal-studio.tsx:65`); the hardcoded English `LEGAL_DOCX_ONLY_MESSAGE` (`apps/web/lib/legal-client.ts:17-18`) is now only the client library's own guard behind it.
