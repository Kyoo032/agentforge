...
- Upload is disabled until both `legal-side-party` and `legal-side-counterparty` are filled; `legal-upload-hint` says so. The first accepted upload creates the matter lazily; a refused file (not `.docx`, over 25 MB) is refused before the matter exists. Each accepted file appears with a role tag that cycles through the roles on click; the click sends `PATCH /api/v1/legal/matters/:matterId` with a `roles` array, not a per-file route.
- `legal-side` and `legal-work-type` are segmented controls; `legal-deliverable-<kind>` are checkboxes; `legal-run` stays disabled until a file is uploaded and both the client party and the counterparty are set.
...
- `legal-studio` carries `data-screen` = `new` | `running` | `result` (`apps/web/components/legal-studio.tsx:294`) — the cheapest assertion for which screen is up.
...
- **Fill the sides first.** Type the client into `legal-side-party` **and** the other side into `legal-side-counterparty` before the first upload. The first upload creates the matter (`ensureMatter`, `apps/web/components/legal-studio.tsx:115-121`), and `POST /api/v1/legal/matters` refuses an empty party or counterparty with `400 invalid_request` (`sideSchema`, `packages/host/src/legal/input.ts:26`). Since the 2026-09-23 fix the renderer asks for both first: with either empty, `legal-drop-zone` and `legal-file-input` are disabled and `legal-upload-hint` reads "Enter the client name and the counterparty before you upload."; nothing is POSTed. A host `invalid_request` that still gets through is shown as `legal.errors.*` copy, never as `side.party is required`.
- **Upload.** Upload `packages/core/src/docx/fixtures/lender-initial-aca-draft.docx` and `depositary-bank-round-1-redline.docx` through `legal-file-input`. With both sides filled: `POST /api/v1/legal/matters` then `POST …/files`, one `legal-file-row` per file, and `legal-run` enables (driven 2026-09-23 with the first fixture). Both appear with counters. Click each role tag to cycle it (PATCH roles): the redline to counterparty draft, the draft to prior turn.
- **Wrong format.** Upload a .txt with both sides filled, before or after a matter exists: the studio refuses it before `POST /api/v1/legal/matters` (`refusedUpload`, `apps/web/components/legal-studio.tsx:67`), so no empty matter is left behind. The exact message “Only .docx files are accepted in v1. Convert PDF or other formats to .docx first.” appears in `legal-error`, `legal-file-row` stays at its previous count, and **no `POST /files` reaches the host** — the check is client-side (`apps/web/lib/legal-client.ts:309-311`). Before a matter exists the sentence comes from `legal.errors.docxOnly` (localized); see [Gotchas](#gotchas).
...
- v1 accepts .docx only. A .txt or .pdf is refused with the exact sentence above — a refusal, not a crash, and nothing joins the matter. The studio's pre-check shows the localized `legal.errors.docxOnly` (`apps/web/components/legal-studio.tsx:67`); the hardcoded English `LEGAL_DOCX_ONLY_MESSAGE` (`apps/web/lib/legal-client.ts:17-18`) is now only the client library's own guard behind it.
...
- The matter is created **lazily** on the first accepted upload (`ensureMatter`, after `refusedUpload`). There is no matter row to assert before a file is dropped; a refused `.txt` no longer leaves an empty matter (fixed 2026-09-23). Clean it up with `DELETE /api/v1/legal/matters/:matterId` — there is no UI control for it.
- **Clean up your matter with the route, not the UI.** `deleteLegalMatter` exists (`apps/web/lib/legal-client.ts:295-298`) but no component calls it; the only references are in its own test. `DELETE /api/v1/legal/matters/:matterId` returns `{ ok: true }` and `rmSync`s the matter folder (`packages/host/src/legal/store.ts:170`) — but it leaves `<dataDir>/legal/<workspaceId>/` behind as an empty directory, so check for the *matter* folder, not the workspace one.
- **Role cycling is a whole-matter PATCH.** There is no `/files/:docId` role route. The tag sends `PATCH /api/v1/legal/matters/:matterId` with `{ roles: [{ id, role }] }` (`apps/web/components/legal-studio.tsx:162`). From the `context` default the first click lands on `counterparty-draft`, then `our-draft`, `prior-turn`, `executed`, `instruction`, `playbook`, `figures`, `precedent`, back to `context` (`nextDocRole`, `apps/web/lib/legal-view.ts:105-108`) — so "the draft to prior turn" is three clicks, not one.
- **`legal-cancel` is a client-side abort**, not a route (`apps/web/lib/use-job-stream.ts:29-32`). There is no cancel endpoint to poke.
- **The per-file cap is 25 MB**, tighter than the 40 MB in `LEGAL_CAPS` because it matches the desktop IPC envelope (`packages/host/src/legal/store-files.ts:20-21`). Matter total is 100 MB and 60 files. A second upload of the same bytes is HTTP 409 `conflict` (sha256 match, `packages/host/src/legal/store.ts:183-190`).
- **A run needs both parties, not a title.** `canRun` is `docs > 0 && partiesReady(draft) && !uploading` (`apps/web/lib/legal-view.ts:114-116`), matching the host's `sideSchema`; an untitled matter is saved as `legal.studio.untitled`.
- `legal-run` stays disabled until a file is uploaded **and** both the client party and the counterparty are set. A disabled Run is that gate, not a hang.
- A stub / no-key run is never a silent no-op, but it is **not an HTTP 503** — `/api/v1/legal/matters/:id/run/stream` answers 200 and `runtime_stub` / `status: 503` ride inside the single `job.error` frame (SKILL.md **Harness-wide gotchas** G2). Two different gates can stop a run: a saved-but-disallowed key trips `requireGatewayAllowed` first and returns a real HTTP 403 `gateway_blocked` with no stream at all (`packages/host/src/handlers/legal.ts:115`); no key at all reaches `requireLive` inside the job (`packages/host/src/legal-generate.ts:68-69`).
- There is no separate Settings link on `legal-error`: `SettingsLinkHint` is appended only when the message does not already match `/settings/i` (`apps/web/components/legal-studio.tsx:355`), and both `stubError` catalogs name Settings in the sentence itself (`packages/core/src/legal/output-copy.ts:65`, id at `:159`), so the guard is always false. Assert the copy, never a hint link.
