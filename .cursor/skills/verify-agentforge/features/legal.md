...
- **Fill the sides first.** Type the client into `legal-side-party` **and** the other side into `legal-side-counterparty` before the first upload. The first upload creates the matter (`ensureMatter`, `apps/web/components/legal-studio.tsx:91-98`), and `POST /api/v1/legal/matters` refuses an empty party or counterparty with `400 invalid_request` (`sideSchema`, `packages/host/src/legal/input.ts:26`). The studio shows the host's raw text in `legal-error` — `side.party is required`, then `side.counterparty is required` — and no file row appears. Driven 2026-09-23; recorded as a product finding.
