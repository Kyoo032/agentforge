...
- Every price chart carries range buttons (1M / 3M / 6M / 1Y / 2Y, default 6M) and plots close + SMA50 + SMA200 on an axis that follows the data rather than anchoring at zero.
...
- **Open Market.** Click `mode-market`. URL matches `/market`. `market-studio` and three `market-starter`; no instruction textarea until `market-options-toggle` is clicked.
