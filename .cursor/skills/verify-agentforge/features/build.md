...
- The redirect is React Router client-side (`apps/web/src/App.tsx:164-165`) and sits next to a catch-all `*` → `/chat` at `:167`. Landing on Chat therefore proves the route is unhandled, not that a dedicated park rule fired — pair the URL assert with `create-agent` count 0.
