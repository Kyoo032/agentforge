...
- The redirect is React Router client-side (`apps/web/src/App.tsx:167-168`) and sits next to a catch-all `*` → `/chat` at `:170`. Landing on Chat therefore proves the route is unhandled, not that a dedicated park rule fired — pair the URL assert with `create-agent` count 0.
