...
- Deleting a desk cascades its threads, messages and runs (`packages/db/src/schema.ts:219-221`), hand-wipes six `knowledge_*` tables (`packages/db/src/ensure-local-owner.ts:176-189`) and drops the desk's `settings.enc` entry. It is not recoverable locally.
