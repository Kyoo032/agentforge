...
- Midjourney-style `mj_*` ids appear in the picker whenever the catalog has them — including with no key. The catalog is `models-cache.json` (`packages/host/src/model-cache.ts:22-30`), written by the last successful `/v1/models` probe and read unconditionally; removing the key does not empty the picker. Only a desk that has *never* had a key (fresh Cloud VM, GHA) shows an empty `images-studio-model`, and that is a different state with the same needs-key banner. Driven 2026-09-17 on `:3000`: `hasOpenai: false`, 40 image ids including fifteen `mj_*`.
