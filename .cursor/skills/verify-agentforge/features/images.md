...
- `images-rail` reaches `/images` from `mode-images` after a custom agent unlocks the surface.
...
- Choose Images on the left rail (`mode-images`) after Build has created an agent with the Images surface (Default template does).
...
- Doctor exits 0.
- `mode-images` is visible. If count is 0, this feature is blocked — report the missing agent/surface. Do not create an agent silently on the operator's desk.
...
- **Cloud.** `foundation.spec.ts` only asserts `images-studio` after Build. It does not generate an image.
