...
- `music-modes` shows `music-studio-mode` with two options, **Describe** and **Custom**, plus `music-studio-mode-hint` explaining which fields the current mode uses. Custom reveals `music-studio-lyrics`, `music-studio-style` and `music-studio-title`; Describe reveals `music-studio-prompt` only. Which fields exist at all is `musicCapabilities(model)` ([`audio-capabilities.ts`](../../../../packages/core/src/models/audio-capabilities.ts)): today only `suno_*` ids carry lyrics / style / title / instrumental, and `resolveMusicMode` silently downgrades Custom to Describe on a model that has no lyrics field. Read the mode back after a model change rather than assuming it stuck.
...
- `music-draft-lyrics` shows `music-studio-draft-lyrics` in Custom mode. It posts `POST /api/v1/music/lyrics` and fills the lyrics box. **Nothing is stored** — no media row, no gallery tile, no Knowledge card — because it exists so the desk can edit the words before spending a music charge.
...
- **Mode switch.** Set `music-studio-mode` to Custom: `music-studio-lyrics`, `music-studio-style` and `music-studio-title` become visible and `music-studio-prompt` goes away. Switch back to Describe and the reverse holds. `music-studio-mode-hint` changes with it. No POST, no key.
