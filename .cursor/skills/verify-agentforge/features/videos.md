...
- **Cost estimate.** With a model selected, `videos-studio-estimate` is visible and starts with `≈ $`. Change `videos-studio-seconds` 5 → 10 and the total doubles; change `videos-studio-resolution` 720p → 480p on a Seedance-class id and both the total and the `$/s` drop. Select `mj_video` and `videos-studio-estimate-unknown` replaces the line. No POST, no key needed.
