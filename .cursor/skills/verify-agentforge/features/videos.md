...
- **Cost estimate.** With a model selected, `videos-studio-estimate` is visible and starts with `≈ $`. Change `videos-studio-seconds` 5 → 10 and the total doubles; change `videos-studio-resolution` 720p → 480p on a Seedance-class id and both the total and the `$/s` drop. Select `mj_video` and `videos-studio-estimate-unknown` replaces the line. No POST, no key, no network: `GET /api/v1/videos` prices the list from the curated table plus an already-cached gateway catalog, and only the POST handler calls `requireGatewayAllowed`. A selected model is the only requirement. Driven 2026-09-15 at 1440x900: `videos-studio-estimate` read `≈ $0.40 for 4 s at 720p · $0.10/s · Google list price, checked Sep 15, 2026 · cheap` and `videos-studio-estimate-compare` read `About 4× cheaper than veo_3_1`.
