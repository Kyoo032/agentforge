...
- `chat-thinking` shows `reasoning-effort` next to the model picker, labeled **Thinking** (`Off` / `Light` / `Normal` / `Deep` / `Extra` / `Max` / `Ultra`). Values stay `none` / `low` / `medium` / `high` / `xhigh` / `max` / `ultra`. Default `medium` (Normal). `Off` skips thinking events. Reasoning models also carry a `model-thinking-badge` in the picker. There is **no** `chat-wire` control.
