...
- `chat-context` shows the Chat header context chip (`chat-context`): estimated tokens in this thread vs the selected model’s `contextLength` (`<used> / <window>`, or `<used> used` if the catalog has no window). Click opens `chat-context-breakdown` with Conversation, Attachments, Knowledge (Soul / Memories / Sources), and Free. Empty thread is `0 / window`.
...
- **Context chip.** `chat-context` is visible in the Chat header. Empty chat with a known window is `0 / <window>`. Click it: `chat-context-breakdown` shows Conversation / Attachments / Knowledge / Free. After messages exist, used tokens are greater than 0.
