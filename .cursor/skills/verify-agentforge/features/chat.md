...
- `chat-thinking` shows `reasoning-effort` next to the model picker, labeled **Thinking** (`Off` / `Light` / `Normal` / `Deep` / `Max`). Values stay `none` / `low` / `medium` / `high` / `ultra`. Default `medium` (Normal). `Off` skips thinking events. Reasoning models also carry a `model-thinking-badge` in the picker. There is **no** `chat-wire` control.
...
- **Open Chat.** Go to `/chat`. `model-picker`, `composer`, `composer-toolbar`, `reasoning-effort`, and `chat-empty` are visible. `reasoning-effort` value is `medium` and shows **Normal**. `chat-wire` count is 0. `chat-empty` headline is `You're in. Ask anything.` `mode-chat` plus the other work modes are visible. `mode-agents` count is 0.
...
- GPT-5.6 Luna/Sol/Terra with Thinking **Off** is coerced to **Light** on Completions/Responses. The picker can still say Off. Messages does not coerce.
- Host auto-routes the POST path. Product Chat does not send `wire` (always auto). GPT-5 / o-series → `/v1/responses`; Claude 5 / Opus 4.7 / 4.8 → `/v1/messages` with the gateway key as `x-api-key` + `anthropic-version: 2023-06-01`; else `/v1/chat/completions`. Probe is still only `GET /v1/models`. Claude 5 never sends `thinking: { type: "enabled", budget_tokens }`. Off → `thinking: { type: "disabled" }`; thinking on → `thinking: { type: "adaptive" }` + `output_config.effort` (Max → `max`).
