...
- `knowledge-loop` on the Sources tab: cycle diagram `knowledge-loop-cycle` (Work → Saved → Indexed → Graph → Retrieved → Verified → Work; `knowledge-loop-stage-<Stage>` with `data-value`, `knowledge-loop-verify` runs the planted-fact self-check — details in [knowledge.md](./knowledge.md)), summary `knowledge-loop-summary` (`knowledge-loop-work`, `-manual`, `-indexed`, `-failed`, `-chunks`, each with `data-value`), and one bar per type `knowledge-loop-count-<Type>` with `data-count` (or `knowledge-loop-empty` when there are no sources). Work types carry an `auto` tag.
