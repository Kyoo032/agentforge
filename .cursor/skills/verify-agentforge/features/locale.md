...
- **Rail walk.** The rail reads `Percakapan` / Chat, `Chat baru`; `Mode kerja` / Dokumen, Riset, Keuangan, Data, Pasar, Hukum, Rapat, Gambar, Video, Musik, Edit, Presentasi; `Akun` / Basis pengetahuan, Kanal, Ruang kerja, Pemakaian, Pengaturan (group labels are sentence case; do not assert capitals — CSS may upper-case them). `mode-*` testids are unchanged.
