...
- `desktop-quit` exits `Agentforge.exe` on window close. Threads and the saved key stay. Uninstall wipes userData + Credential Manager wrap key.
...
- **Uninstall:** NSIS `customUnInstall` taskkills `Agentforge.exe`, `RMDir` `%APPDATA%\Agentforge`, and `cmdkey /delete:Agentforge/wrap-key`. Reinstall must show onboarding.
