...
- `meeting-create` — `meeting-new`, `meeting-title`, `meeting-locale` (a full-width `<select>`: English / Indonesian — the 0.15.0 layout fix stacks the caption above it), `meeting-create`. The meeting appears in `meeting-list` as `meeting-item-<id>` with `meeting-delete-<id>`; `meeting-empty` shows while the list is empty.
...
4. `meeting-delete-<id>` removes it — **immediately, with no confirm dialog** (driven 2026-09-23). Clean up every meeting you create and confirm `GET /api/v1/meetings` → `items` is back to its starting length.
...
- **Delete does not ask.** `meeting-delete-<id>` removes the meeting and its folder on the first click. Do not press it on a meeting you did not create.
