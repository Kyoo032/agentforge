...
- `rail-sessions` is the CONVERSE session block's **chrome**: `rail-thread-list` exists only on an expanded rail, wraps `new-chat-link`, holds up to four `thread-item` rows (`RAIL_RECENT_THREADS = 4`, `apps/web/lib/thread-groups.ts:2`), and shows `threads-see-all` when the host returned more than four and `rail-thread-error` when the list load or a delete failed. This is the product's only session list. What a row does — open, delete, the confirm copy, the fork race — belongs to [chat.md](./chat.md); this file owns only "the block is there, and it disappears when the rail collapses".
...
- `rail-collapse` / `rail-expand` is one button with a flipping testid: `rail-collapse` while expanded, `rail-expand` while collapsed (`apps/web/components/app-rail.tsx:416`). It writes `agentforge-rail-collapsed` (`"1"` / `"0"`) and sets `data-rail="min"` / `"full"` on the `<aside>`.
...
- `rail-theme` is `theme-toggle` in the footer: it flips the `dark` class on `<html>` and stores `agentforge-theme` (`light` / `dark`). `aria-label` reads the target mode, not the current one.
...
- **Sessions.** `rail-thread-list` count 1, `new-chat-link` count 1. On a desk with history, `thread-item` > 0 and `threads-see-all` count 1; on a fresh desk both are 0 and that is the pass. Opening, deleting and the send-too-early race are driven in [chat.md](./chat.md), not here.
...
- **Theme.** Read `document.documentElement.className` and `localStorage["agentforge-theme"]`. Click `theme-toggle`: the class gains `dark` and the key reads `dark`. Click again: class clears, key reads `light`. There is no `data-theme` attribute — do not assert one. Leave the operator's theme as you found it.
