...
- `rail-collapse` / `rail-expand` is one button with a flipping testid: `rail-collapse` while expanded, `rail-expand` while collapsed (`apps/web/components/app-rail.tsx:408`). It writes `agentforge-rail-collapsed` (`"1"` / `"0"`) and sets `data-rail="min"` / `"full"` on the `<aside>`.
