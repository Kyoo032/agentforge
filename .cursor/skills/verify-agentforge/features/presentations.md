...
- `presentations-shapes` inserts `presentations-add-rectangle`, `-rounded`, `-ellipse`, `-triangle`, `-line`, `-arrow`, `-star`, `-callout`, and `-text`. Each mark is `presentations-shape` (`data-x`, `data-y`, `data-w`, `data-h`). Select it, drag the box, or drag `presentations-shape-resize`. Fill and stroke are `presentations-fill` and `presentations-stroke`. `presentations-shape-remove` deletes it, and so do Delete and Backspace when focus is not in a text field. Arrow keys nudge by 1; Shift+arrow nudges by 5. `presentations-shape-duplicate` copies the selection slightly down and right and selects the copy. `presentations-edit-note` says tables, charts, and masters are not on this surface.
...
- **Stage.** After a starter, `presentations-filmstrip` lists the title plus each content slide. Edit `presentations-edit-title` on the title slide. Open the first content slide and edit `presentations-edit-heading` and one `presentations-edit-bullet`. Click `presentations-add-rectangle` and `presentations-add-ellipse`. Select the rectangle: ArrowRight moves `data-x` by 1, Shift+ArrowRight by 5. `presentations-shape-duplicate` adds a copy; Delete removes the copy and leaves the original. Drag the remaining rectangle and resize the ellipse with `presentations-shape-resize`. Click `presentations-save-deck`. `presentations-deck-saved` appears. Reload: `presentations-studio-empty` is back. Click `presentations-deck-open`, then the same filmstrip entry: the heading and the surviving shapes (`data-x`, `data-y`, `data-w`, `data-h`) are back. Download again: the PPTX slide XML contains `prst="rect"` and `prst="ellipse"`.
