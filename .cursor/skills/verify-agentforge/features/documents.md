...
- `documents-studio-model` is a real `<select>` carrying the whole curated chat catalog (110 options on this desk; `packages/host/src/selectable-models.ts:166`), and it writes the desk's `documentGenModel` — the same field Finance, Market and Legal read (`apps/web/lib/use-job-model.ts:19-25`). Changing it here changes Finance's default too.
