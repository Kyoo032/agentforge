...
- `documents-prompt` + `documents-generate` are the generate bar (`documents-studio-prompt-bar`). Generate goes quiet, and `documents-needs-key` appears, only when `gateway.allowed` is false. On stub (`allowed: true`) the button stays enabled and `documents-needs-key` has count 0, but the host still answers HTTP 503 `runtime_stub` into `documents-error` — and that failure also clears any starter draft on screen.
...
- A failed generate wipes the preview. `onGenerate`'s catch does `setDraft(null)` before it sets the error (`apps/web/components/documents-studio.tsx:83-85`); `onRegenerate`'s catch does not. Load a starter, then press Generate without a key, and the starter is gone.
- `documents-studio-model` is a real `<select>` carrying the whole curated chat catalog (110 options on this desk; `packages/host/src/selectable-models.ts:167`), and it writes the desk's `documentGenModel` — the same field Finance, Market and Legal read (`apps/web/lib/use-job-model.ts:19-25`). Changing it here changes Finance's default too.
