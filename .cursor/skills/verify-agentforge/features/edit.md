...
- **Prompt templates.** Open a project, `edit-generate-tab` › Video. `edit-prompt-templates` lists 19 templates; click `edit-prompt-template-category-reel` then `edit-prompt-template-reel-runway-pan`. Prompt textarea now reads "At a fashion show scene, the camera pans right…", seconds select is 5, `edit-prompt-template-source` shows "From BytePlus: Seedance-1.0-pro&pro-fast prompt guide" with a docs.byteplus.com link. `edit-prompt-guide` expands to the rules list. Typing in the prompt clears the selection. Do not press Generate on the operator desk.
- **Starter media.** Pick `Product promo` in `edit-starter` (`edit-starter-description` reads "Title card, three sample shots…"), `edit-new-project`. Timeline shows a title clip then 3 `edit-clip` on `v1` and 1 on `a1`; `ops 0` (seeded into the snapshot, not the ops log). Precondition: `pnpm edit:starters:check` passes and the host was started after the starter files existed. Blank starters stay empty.
