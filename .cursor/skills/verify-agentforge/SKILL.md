...
| `model-picker`, `model-picker-panel`, `model-group-recommended`, `model-best-for`, `model-thinking-badge`, `composer`, `composer-text`, `composer-attach`, `composer-send`, `composer-enhance` | Chat. The picker panel is portalled to `document.body`; options are `[role="option"]` with no per-row testid. `composer-send-hint` has count 0 since 0.15.0 |
| `chat-key-status`, `chat-whats-this`, `chat-whats-this-body`, `chat-suggestions`, `chat-suggestion` (four, `data-idea` `plan` / `summarise` / `rewrite` / `decide`) | Chat empty state (0.15.0): status line, and intent cards that **fill** the composer, never send — [chat.md](features/chat.md) |
