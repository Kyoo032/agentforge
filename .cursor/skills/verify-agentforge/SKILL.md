...
| `rail-finance-mode`, `finance-tasks-toggle`, `rail-finance-tasks`, `rail-finance-tasks-branch`, `finance-task-<id>` | Finance tasks under the rail's Finance row. Both submenu chevrons are arrow-only (label in `aria-label` / `title`) — [rail.md](features/rail.md), [finance.md](features/finance.md) |
