...
- **Product** is `apps/`, `packages/`, Chat / Settings / Workspaces / job modes — two products on one renderer since 2026-09-23 (`AGENTS.md`, top table): **Personal** is the Electron desktop app (installer, IPC, no HTTP port; current cut **0.15.0**, under active development again) and **Enterprise** is the hosted multi-user web app. A UI change in `apps/web` usually lands on both; packaging, identity and deploy land on one. Decision record: [`web-pivot-2026-09-18.md`](../../../docs/internal/web-pivot-2026-09-18.md); cut checklist: [`0.15.0-changelog.md`](../../../docs/internal/0.15.0-changelog.md).
...
**Surfaces.** Two surfaces share UI code; they do **not** share a port. Webdev `:3000` is where every UI and host change is driven first, for both products. The hosted environment is the Enterprise proof path once it exists. `--desktop` is the proof path for the **Personal** app (0.15.0): nothing counts as shipped on it until it is packed, installed and doctored with `--desktop`, and it is the only thing that proves a packaged shell change.

| Surface | How to reach it | Doctor | Port |
|---|---|---|---|
| **Local webdev** | `pnpm --filter @agentforge/web dev` → Chrome / IDE browser | `node .cursor/skills/verify-agentforge/scripts/doctor.mjs` | **3000 only** (`tsx server.ts` → Express + Vite on `127.0.0.1:3000`) |
| **Packaged desktop (Personal, 0.15.0)** | Installed DPSBuddy, or local Kemenkeu AI / AIHub Metranet (Windows NSIS; mac/linux operator-built) | `node .cursor/skills/verify-agentforge/scripts/doctor.mjs --desktop` | **None.** IPC only. Status in userData `host-status.json` (Windows `%APPDATA%\DPSBuddy` or `%APPDATA%\Kemenkeu AI` / `%APPDATA%\AIHub Metranet`; Linux `$XDG_CONFIG_HOME/DPSBuddy` or `~/.config/DPSBuddy`; macOS `~/Library/Application Support/DPSBuddy`) |
...
| This Windows session (Claude Code) | **`http://127.0.0.1:3000` is the owner's isolated webdev — drive it**: throwaway data, locale switches and mutating steps allowed. It **may hold a real gateway key** — read doctor's `runtime` / `hasOpenai` first. With `runtime: "ai"` every model call is billed: drive surfaces (navigation, testids, disclosures, validation, local imports), allow at most one short Chat send as the live proof, and record every generate / run / transcription / embedding step as "not driven (paid)" unless the owner asked. On a live desk, creating a desk strands the app on onboarding ([workspaces.md](features/workspaces.md) Gotchas). Doctor first (`scripts/doctor.mjs`), then a short scratch Playwright script. Clean up every thread, desk, dataset, matter and file you create; evidence is never deleted. `tsx server.ts` has **no watcher**, so a host or core edit made today is only live on a process started after it — an agent here cannot kill node, so say "restart :3000" and let the owner do it, rather than starting a rival. Start a second instance on a free port only when :3000 does **not** answer, or when you need a state :3000 cannot reach (a closed gate needs `AGENTFORGE_RUNTIME=ai` and its own `AGENTFORGE_DATA_DIR`), and stop it yourself. | Start a second instance while :3000 answers; press `settings-reset*`; type a real gateway key; leave a drive script in `scripts/`; `pnpm test:e2e` |
| Cursor Cloud | Playwright `apps/web/tests/e2e/foundation.spec.ts` | Paste a gateway key; start Docker |
| Pack / ship (this PC only) | [pack-dpsbuddy](../pack-dpsbuddy/SKILL.md) | Cloud `desktop:build`; `electron-builder --mac` on Windows |

Cloud owns the serial stub smoke; there is no GitHub Actions job any more (deleted 2026-09-24 — CI is `pnpm ci:local`, and its Playwright step runs only with `--e2e`). Local coding agents do not run Playwright here by default (long serial pass). Cloud may use a secret gateway key for live Enhance / Finance / Data proof after doctor `ai`; the `ci:local --e2e` run forces stub.
...
Root `pnpm dev` is `turbo dev`, which also starts `apps/portal`. Without `PORTAL_DATABASE_URL` the portal exits 2 and turbo stops the web app with it (driven 2026-09-26: portal "configuration is not usable", web never bound :3000). A UI drive does not need the portal. Playwright's `webServer` is `pnpm dev` inside `apps/web` (`tsx server.ts`), which is the same process as the filter above.
...
**Cloud boot** already runs `scripts/cloud-start.sh` (data-dir prep only: `mkdir data`, unset `DATABASE_URL`). Then `pnpm --filter @agentforge/web dev`. Playwright `webServer` in `apps/web/playwright.config.ts` starts `pnpm dev` from `apps/web` with `AGENTFORGE_RUNTIME=stub` and `AGENTFORGE_DATA_DIR` resolved to repo `data/`. `reuseExistingServer` is on unless `CI`.

**Stub vs live.** `AGENTFORGE_RUNTIME=stub` until a gateway (or other provider) key is saved. A saved key makes `GET /api/v1/settings` report `runtime: "ai"`. `ci:local --e2e` stays stub with no key. Cursor Cloud may have a secret `OPENAI_API_KEY` for live loop proof — doctor `runtime: "ai"` and `hasOpenai: true` before a live send; if the env is missing, stay stub. This Windows machine may already be live — doctor first. Never echo the key.
...
- `--desktop` finds the `pid` in `host-status.json` not running. The file outlives the app, so a closed or crashed DPSBuddy leaves `ready: true` behind. Since 2026-09-23 doctor checks the pid with `process.kill(pid, 0)` and fails on a dead one (before that it reported `ok: true` for a stale file — driven: pid 2084 absent, doctor said ok)
...
| testid | Surface |
|---|---|
| `mode-chat`, `mode-documents`, `mode-research`, `mode-finance`, `mode-data`, `mode-market`, `mode-legal`, `mode-meeting`, `mode-images`, `mode-videos`, `mode-music`, `mode-presentations`, `mode-edit`, `mode-education` | Left rail work modes (Default has all of these). `mode-edit` shipped in 0.14.22. `mode-education` is the teaching desk |
...
| `model-picker`, `model-picker-panel`, `model-group-recommended`, `model-best-for`, `model-thinking-badge`, `composer`, `composer-text`, `composer-attach`, `composer-send`, `composer-enhance`, `composer-needs-key` | Chat. The picker panel is portalled to `document.body`; options are `[role="option"]` with no per-row testid. `composer-send-hint` has count 0 since 0.15.0. `composer-needs-key` and a disabled Send appear only when `gateway.allowed` is false. Stub is `allowed: true`, so Send stays enabled and `composer-needs-key` has count 0 — [chat.md](features/chat.md) |
| `chat-key-status`, `chat-empty-settings`, `chat-whats-this`, `chat-whats-this-body`, `chat-suggestions`, `chat-suggestion` (four, `data-idea` `plan` / `summarise` / `rewrite` / `decide`) | Chat empty state: status line, a Settings link (`chat-empty-settings`, `btn-primary` only while `allowed` is false), and intent cards that **fill** the composer, never send — [chat.md](features/chat.md) |
| `message-thinking`, `thinking-placeholder`, `message-tools`, `message-tool` | One turn's Thinking disclosure; tool rows live **inside** it since 0.15.0 and it exists only when the turn has reasoning or tools |
| `rail-finance-mode`, `finance-tasks-toggle`, `rail-finance-tasks`, `rail-finance-tasks-branch`, `finance-task-<id>` | Finance tasks under the rail's Finance row. Both submenu chevrons show a short word (`tasks` / `specialists`) and keep the full phrase in `aria-label` / `title` — [rail.md](features/rail.md), [finance.md](features/finance.md) |
...
| `chat-empty`, `message-list`, `message-output`, `rail-thread-list`, `thread-item`, `rail-thread-error`, `new-chat-link`, `threads-see-all`, `thread-delete`, `chat-error`, `composer-error` | Threads (there is no header `new-chat`) + assistant markdown output; `rail-thread-error` is the only symptom of a failed list load or a refused delete; live contact fail after 3 tries |
...
| `settings-reset`, `settings-reset-danger`, `settings-reset-key`, `settings-reset-key-confirm`, `settings-reset-key-submit`, `settings-reset-all`, `settings-reset-all-confirm-name`, `settings-reset-all-submit`, `settings-reset-cancel`, `settings-reset-pending` | Start over card. Sign-out and delete sit inside the closed `settings-reset-danger` disclosure (`Sign out and delete`). Open it before asserting those rows are visible; a closed `<details>` keeps them in the DOM. **Never press the submits on a desk you do not own** |
| `onboarding-welcome`, `onboarding-next`, `onboarding-back`, `onboarding-steps`, `onboarding-form`, `onboarding-key`, `onboarding-continue`, `onboarding-gate-reason`, `onboarding-recheck`, `onboarding-setup-check`, `onboarding-try`, `onboarding-example`, `onboarding-start`, `onboarding-key-success` | First-run / gate-closed screen. Hello, then the key, then four examples that open Chat. The screen scrolls itself (`h-full overflow-y-auto`). `onboarding-gate-reason` + `onboarding-recheck` appear only when there is a failure to explain. `onboarding-endpoint` and `onboarding-gateway-host` must have count 0 — the address is not shown |
| `rail-footer`, `theme-toggle`, `app-updates`, `app-updates-toggle`, `app-updates-badge`, `app-updates-panel`, `app-updates-status`, `app-updates-check`, `app-updates-install`, `app-updates-close`, `rail-collapse` / `rail-expand` (one button, testid flips by state), `rail-resize` (expanded only) | Rail footer: theme icon, collapse, resize. The updates icon renders only in the packaged public DPSBuddy shell (`isElectron()`). Webdev and the hosted web hide it — `app-updates` count 0. Labels stay on the rows down to 480px; at 480px or narrower the rail starts collapsed — [rail.md](features/rail.md) |
| `usage-range-empty`, `usage-desk-range`, `usage-by-model`, `usage-key-meter` | Usage page (by-model + desk range; empty chart copy) |
| `images-studio`, `images-studio-needs-key`, `images-studio-submit`, `documents-generate`, `documents-needs-key`, `videos-studio`, `videos-studio-needs-key` | Images and Documents quiet Generate only when `allowed` is false. The Images needs-key note still follows `GET /api/v1/images` `ready`, so on a keyless stub desk the note can show while Generate stays enabled. Videos (and Music) stay on the route `ready` flag: no key means the needs-key note and a quiet Generate — [images.md](features/images.md), [documents.md](features/documents.md), [videos.md](features/videos.md) |
...
| `knowledge-loop`, `knowledge-loop-stage-<Stage>`, `knowledge-loop-summary`, `knowledge-loop-work`, `knowledge-loop-count-<Type>`, `knowledge-loop-empty`, `knowledge-loop-verify`, `knowledge-loop-verified`, `knowledge-loop-verify-error`, `knowledge-source-type` | Knowledge health strip (Sources tab): tiles Work / Saved / Indexed / Retrieved plus the Verified pill; `knowledge-loop-cycle` and a Graph stage no longer exist — [knowledge-ingest.md](features/knowledge-ingest.md), [knowledge.md](features/knowledge.md) |
...
Recipes (every file in `features/` except the index): [features/channels.md](features/channels.md), [features/chat.md](features/chat.md), [features/components.md](features/components.md), [features/data.md](features/data.md), [features/desktop-brands.md](features/desktop-brands.md), [features/desktop.md](features/desktop.md), [features/documents.md](features/documents.md), [features/edit.md](features/edit.md), [features/education.md](features/education.md), [features/finance.md](features/finance.md), [features/gateway-gate.md](features/gateway-gate.md), [features/images.md](features/images.md), [features/knowledge-graph.md](features/knowledge-graph.md), [features/knowledge-ingest.md](features/knowledge-ingest.md), [features/knowledge-phases.md](features/knowledge-phases.md), [features/knowledge.md](features/knowledge.md), [features/legal.md](features/legal.md), [features/locale.md](features/locale.md), [features/login.md](features/login.md), [features/market.md](features/market.md), [features/meeting.md](features/meeting.md), [features/mobile.md](features/mobile.md), [features/models.md](features/models.md), [features/music.md](features/music.md), [features/pii.md](features/pii.md), [features/plans.md](features/plans.md), [features/presentations.md](features/presentations.md), [features/rail.md](features/rail.md), [features/research.md](features/research.md), [features/security.md](features/security.md), [features/settings.md](features/settings.md), [features/templates.md](features/templates.md), [features/usage.md](features/usage.md), [features/videos.md](features/videos.md), [features/workspaces.md](features/workspaces.md). [login.md](features/login.md) and [plans.md](features/plans.md) are the **Enterprise** (hosted) lane and are verified-unreachable on `:3000` and the desktop. [build.md](features/build.md) and [studio-advanced.md](features/studio-advanced.md) are parked (both redirect to Chat).
...
**G6 — Motion is a context setting, not a CSS hack.** 0.15.0 ends `apps/web/app/globals.css` with one `@media (prefers-reduced-motion: reduce)` block that sets every transition and animation to `0.01ms`. Prove it with `browser.newContext({ reducedMotion: "reduce" })`, then read `getComputedStyle(el).transitionDuration` on `.composer-shell`, `.btn`, `.wash` or `.transition-colors`: `1e-05s` under reduce, `0.12s` (the `--motion-1` step) under `no-preference`. Driven 2026-09-23 on `/chat`. The same setting is why a screenshot under reduce never catches a mid-transition frame.
## Evidence
